...
class DatalakeAdapter:
    """Reads canonical market history from tables or parquet exports."""
    ...
    def normalize_symbol_universe(self, *, symbols: list[str], source: dict[str, Any]) -> list[str]:
        ...
        if mode == "canonical_tables":
            ...
            rows = self._connection.execute(
                ...
                SELECT
                    ...
                    COALESCE(b.volume_adj, b.volume_raw) AS volume
            ...
            rows = self._connection.execute(
                ...
                bars AS (
                    SELECT
                        ticker_id,
                        trading_date,
                        COALESCE(close_adj, close_raw) AS close_price,
                        COALESCE(volume_adj, volume_raw) AS volume
                    FROM read_parquet(?)
                )
                SELECT t.symbol, b.trading_date, b.close_price, b.volume
                FROM bars AS b
